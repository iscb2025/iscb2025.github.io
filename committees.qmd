...
print_committee <- function(data, title, cols=c("name")) {
...
committees |> 
  filter(loc_chair == 1) |> 
  print_committee(title=c("Chairs" = 1))
```

```{r, echo=F, include=T}
committees |> 
  filter(finance == 1) |> 
  print_committee(title=c("Finance" = 1))
```

```{r, echo=F, include=T}
committees |> 
  filter((loc_core == 1 | loc_extended == 1) & is.na(loc_chair) & is.na(finance)) |> 
  print_committee(title=c("Members" = 1))
```
:::
::: {.g-col-12 .g-col-md-6}
...
committees |> 
  filter(spc_chair == 1) |>
  print_committee(title=c("Chair" = 2), cols = c("name", "country"))
```

```{r, echo=F, include=T}
committees |> 
  filter(spc == 1 & is.na(spc_chair)) |>
  print_committee(title=c("Members" = 2), cols = c("name", "country"))
```
:::
::: 
